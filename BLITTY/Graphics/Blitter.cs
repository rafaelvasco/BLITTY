...
public static unsafe class Blitter
{
    ...
    private static Texture2D? _textureTarget;
    ...
    private static void SetDrawState(byte[] pixels, int surfaceWidth, int surfaceHeight)
    {
        ...
        _clipRect = Rect.FromBox(0, 0, _surfaceW, _surfaceH);
        _ready = true;
    }
    ...
    public static void Clip(Rect rect)
    {
        ...
        else if (!(Rect.FromBox(0, 0, _surfaceW, _surfaceH)).Contains(_clipRect))
        {
            _clipRect = Rect.FromBox(0, 0, _surfaceW, _surfaceH);
        }
    }
    ...
    )
    {
        ...
        if (region.IsEmpty)
        {
            region = Rect.FromBox(0, 0, pastePixelsW, pastePixelsH);
        }
        ...
        fixed (byte* paste = &MemoryMarshal.GetReference(pastePixels))
        {
            if (!flip)
            {
                for (int px = min_x; px < max_x; ++px)
                {
                    ...
                    {
                        byte* src_idx = paste + (region.X1 + (int)((px - x) / factor_w) + (region.Y1 + (int)((py - y) / factor_h)) * paste_w) * 4;
            ...
            else
            {
                ...
                for (int px = min_x; px < max_x; ++px)
                {
                    for (int py = min_y; py < max_y; ++py)
                    {
                        byte* src_idx = paste + (start_pix_x + (int)((px - x) / factor_w) + (region.Y1 + (int)((py - y) / factor_h)) * paste_w) * 4;
