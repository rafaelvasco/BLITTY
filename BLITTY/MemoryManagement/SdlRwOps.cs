...
namespace BLITTY.MemoryManagement
{
    internal class SdlRwOps : DisposableResource
    {
        ...
        private unsafe SDL_RWops* _rwOps;
        ...
        internal SdlRwOps(Stream stream, bool keepOpen = false)
        {
            ...
            unsafe
            {
                _rwOps = (SDL_RWops*)SDL_AllocRW();

                if (_rwOps == null)
                {
                    throw new BlittyException($"Failed to allocate SDL RWops: {SDL_GetError()}");
                }
        ...
        private long Seek(IntPtr context, long offset, int whence)
        {
            if (!_stream.CanSeek)
            {
                SDL_SetError("This does not support seeking.");
                return -1;
            }
        ...
        private unsafe ulong Read(IntPtr context, void* ptr, ulong size, ulong maxnum)
        {
            if (!_stream.CanRead)
            {
                SDL_SetError($"Attempted to read from a write-only stream.");
                return 0;
            }
        ...
        private unsafe ulong Write(IntPtr context, void* ptr, ulong size, ulong maxnum)
        {
            if (!_stream.CanWrite)
            {
                SDL_SetError("Attempted to write to a read-only stream.");
                return 0;
            }
        ...
        private int Close(IntPtr context)
        {
            ...
            {
                SDL_SetError("Attempted to free a null RWops handle.");
                return -1;
            }
